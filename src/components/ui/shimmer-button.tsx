...
export interface ShimmerButtonProps extends ComponentPropsWithoutRef<"button"> {
  ...
  /** When true, renders the single child element (e.g. an <a>) with shimmer styles instead of a button */
  asChild?: boolean
}
...
>(
  (
    ...
    if (asChild && React.isValidElement(children)) {
      const child = children as React.ReactElement<{ className?: string; style?: CSSProperties; ref?: React.Ref<unknown> }>
      return React.cloneElement(child, {
        className: cn(sharedClassName, child.props.className),
        style: { ...style, ...child.props.style },
        ref: child.ref ?? ref,
      }, shimmerContent, child.props.children)
    }
